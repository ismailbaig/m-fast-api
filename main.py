...
from db_mongo.register_and_login import insert_register_user, login_user
...
async def login_user_api(user: LoginModel):
    # Find user by email
    found_user = await login_user(user)

    if found_user is None:
        raise HTTPException(status_code=404, detail="Invalid Crediential")
